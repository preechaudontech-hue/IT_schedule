...
- **08:00 น. (เช้า)** — สรุปภารกิจตั้งแต่วันนี้เป็นต้นไป (วันนี้ + วันถัดไปทั้งหมด) พร้อมหัวข้อ **"📌 อย่าลืม / สิ่งที่ต้องปฏิบัติ"**
  ต่อท้าย (งานที่ไม่มีวันที่/เวลาตายตัว) — ดูกติกาการงดส่งวันเสาร์-อาทิตย์ด้านล่าง
- **11:30 น. (เที่ยง)** — เตือนเฉพาะภารกิจ**ช่วงบ่ายของวันนี้** (เวลา ≥ 12:00) พร้อม "อย่าลืม" ต่อท้ายเช่นกัน

**กติกาการส่ง/งดส่ง** (ใช้เหมือนกันทั้ง 2 รอบ):
| มีภารกิจในตาราง (ของรอบนั้น) | มีแต่ "อย่าลืม" (ไม่มีภารกิจ) | ไม่มีทั้งคู่ |
|---|---|---|
| ส่งเสมอ ทุกวันรวมเสาร์-อาทิตย์ | จันทร์-ศุกร์: ส่ง (อย่าลืมอย่างเดียว) · **เสาร์-อาทิตย์: งดส่ง** | งดส่ง |
...
4. สร้างแท็บที่สองชื่อ `todos` (ในสมุดเดียวกัน) สำหรับหัวข้อ **"อย่าลืม / สิ่งที่ต้องปฏิบัติ"**
   — งานที่ไม่มีวันที่/เวลาตายตัว จะถูกแจ้งซ้ำทุกเช้าจนกว่าจะติ๊กว่าเสร็จผ่านหน้าเว็บ
5. แถวแรกของแท็บ `todos` เป็นหัวคอลัมน์: `รายการ | ผู้รับผิดชอบ | เสร็จสิ้น | หมายเหตุ | วันที่เพิ่ม`
   — คอลัมน์ `เสร็จสิ้น` แนะนำให้ทำเป็น checkbox จริง (เลือกคอลัมน์ → เมนู **Insert → Checkbox**)

| รายการ | ผู้รับผิดชอบ | เสร็จสิ้น | หมายเหตุ | วันที่เพิ่ม |
|---|---|---|---|---|
| ส่งเอกสารงบประมาณ | อ.ปรีชา | ☐ | | 2026-09-17 |
...
## ปรับแต่ง
- ค่าเริ่มต้น (รอบเช้า) คือ `SEND_WHEN_EMPTY=0` — วันไหนไม่มีงานในตารางเลย จะไม่ส่งข้อความสรุปตาราง
  ถ้าอยากให้ส่งข้อความ "ไม่มีกำหนดการ" ทุกวันแทน ให้แก้ `env` ใน `.github/workflows/daily-notify.yml` เป็น `SEND_WHEN_EMPTY: "1"`
- งดส่งวันเสาร์-อาทิตย์เมื่อมีแต่ "อย่าลืม" ไม่มีภารกิจในตาราง — เป็น logic ในโค้ด (`compose_or_skip` ใน `src/notify.py`)
  ไม่ใช่ env var ถ้าอยากเปลี่ยนวันที่ถือเป็น "วันหยุด" แก้เงื่อนไข `target.weekday() >= 5` ในฟังก์ชัน `main()`
